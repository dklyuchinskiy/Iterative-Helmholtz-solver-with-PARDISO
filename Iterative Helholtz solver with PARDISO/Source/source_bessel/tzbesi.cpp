...
#include "complex.h"
  ...
  void ZBESI(REAL ZR, REAL ZI, REAL FNU, int KODE, int N, REAL *CYR, REAL *CYI,
	  int *NZ, int *IERR) {
	  ...
	  //Labels: e40, e120, e130, e260

	  REAL  AA, ALIM, ARG, CONEI, CONER, CSGNI, CSGNR, DIG, ELIM,
		  FNUL, RL, R1M5, STR, TOL, ZNI, ZNR, AZ, BB, FN;
	  int   I, INU, K, K1, K2, NN;

	  //***FIRST EXECUTABLE STATEMENT  ZBESI
	  CONER = ONE; CONEI = ZERO;
	  *IERR = 0;
	  *NZ = 0;
	  if (FNU < ZERO)  *IERR = 1;
	  ...
	  CSGNR = -CSGNR;
	  CSGNI = -CSGNI;
  e40:  ZBINU(ZNR, ZNI, FNU, KODE, N, CYR, CYI, NZ, RL, FNUL, TOL, ELIM, ALIM);
	  if (*NZ < 0) goto e120;
	  if (ZR >= ZERO)  return;
	  /*----------------------------------------------------------------------
